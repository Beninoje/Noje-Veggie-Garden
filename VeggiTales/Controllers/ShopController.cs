...
namespace VeggiTales.Controllers
{
    public class ShopController : Controller
    {
        ...
        public async Task<IActionResult> Search(string searchTerm)
        {
            ...
            {
                viewModel.Categories = await _context.Categories.Include(c => c.Products).ToListAsync();
            }
            else
            {
                viewModel.Products = await _context.Products.Where(p => p.Name.Contains(searchTerm)).ToListAsync();
        ...
        // POST: /Shop/Favourites => process request to add an item to the user's cart
        [HttpPost]
        [ValidateAntiForgeryToken]  // cross-site script protection
        public IActionResult AddToFavourites(int ProductId)
        {
            ...
            // check if product is already in this user's cart
            var favouriteItem = _context.FavoriteItems
                .Where(c => c.ProductId == ProductId && c.CustomerId == customerId)
                .FirstOrDefault();
            ...
            if (favouriteItem == null)
            {
                ...
                _context.FavoriteItems.Add(favouriteItem);
            }
/*            else
            {
                favouriteItem.Quantity += Quantity;
                _context.CartItems.Update(cartItem);
            }*/
        ...
        public IActionResult Favourites()
        {
            ...
            // fetch the items in this cart
            var favouriteItems = _context.FavoriteItems
                .Include(c => c.Product)
                .Where(c => c.CustomerId == customerId);
        ...
        {
            // get selected cart item
            var favouriteItem = _context.FavoriteItems.Find(id);
